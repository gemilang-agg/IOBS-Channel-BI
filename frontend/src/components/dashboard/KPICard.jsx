...
function TrendIndicator({ isPositive, change, comparisonLabel = 'vs last period' }) {
  ...
  return (
    <>
      ...
      <span className="text-xs kiosk-sm:text-sm kiosk:text-base text-slate-400 ml-1 truncate">
        {comparisonLabel}
      </span>
    </>
  );
      ...
      <div className="flex items-start justify-between">
        <div className="space-y-1 min-w-0">
          ...
          {comparisonValue !== undefined && (
            <p
              className="text-[11px] kiosk-sm:text-xs text-slate-400 dark:text-slate-500 tabular-nums"
              data-testid={`kpi-card-${label.toLowerCase().replace(/\s+/g, '-')}-comparison`}
            >
              was {formatValue(comparisonValue, unit)}
            </p>
          )}
        </div>
        {Icon && (
          <div className="w-10 h-10 kiosk-sm:w-12 kiosk-sm:h-12 kiosk:w-14 kiosk:h-14 rounded-lg bg-blue-50 dark:bg-blue-500/10 flex items-center justify-center shrink-0">
